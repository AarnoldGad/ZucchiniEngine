...
class Tee<PrintFn, ze::if_is_invocable<PrintFn> >
{
public:
...
class Tee<Stream, ze::if_is_stream<Stream> >
{
public:
   template<typename Value>
   decltype(auto) operator()(Value&& value);
...
template<typename PrintFunc, ze::if_is_invocable<PrintFunc, int> = 0>
Tee<PrintFunc> make_tee(PrintFunc func) noexcept;

template<typename Stream, ze::if_is_stream<Stream, int> = 0>
