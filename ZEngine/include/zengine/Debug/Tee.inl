...
decltype(auto) Tee<PrintFn, ze::if_is_invocable<PrintFn> >::operator()(Value&& value)
{
   m_print(value);
...
Tee<Stream, ze::if_is_stream<Stream> >::Tee(Stream& output) noexcept
   : m_output(output) {}

template<typename Stream>
template<typename Value>
decltype(auto) Tee<Stream, ze::if_is_stream<Stream> >::operator()(Value&& value)
{
   m_output << value << std::endl;
   return std::forward<Value>(value);
}

template<typename PrintFunc, ze::if_is_invocable<PrintFunc, int>>
Tee<PrintFunc> make_tee(PrintFunc func) noexcept
{
   return Tee<PrintFunc>(func);
}

template<typename Stream, ze::if_is_stream<Stream, int>>
